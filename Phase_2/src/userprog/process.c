...
process_execute (const char *file_name) 
{
  ...
  if (!thread_current()->successCreation) 
  {
    return TID_ERROR;
  }
...
start_process (void *file_name_)
{
  ...
  if (success) 
  {
    parent->successCreation = true;
    list_push_back(&parent->childList,&child->child_elem);
    sema_up(&parent->sync);
...
process_wait (tid_t child_tid) 
{
  ...
  for (struct list_elem* e = list_begin (&cur->childList); e != list_end (&cur->childList);
  e = list_next (e))
  {
...
process_exit (void)
{
  ...
  while (!list_empty(&cur->filesList))
  {
    struct open_file* opened_file = list_entry(list_pop_back(&cur->filesList), struct open_file, elem);
  ...
  {
    struct thread* child = list_entry(list_pop_back(&cur->childList), struct thread, child_elem);
    child->parent = NULL;
    sema_up(&child->sync);
  }
