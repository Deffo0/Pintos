...
#include "threads/fixed-point-math.c"
...
void thread_set_priority(int new_priority)
{
    ...
    {
        list_sort(&ready_list, &readylist_greater_comp, NULL);
...
void thread_set_nice(int nice UNUSED)
{
    ...
    int recCpuOv4 = ((thread_current()->recentCpu.recentCpu) / 4);
...
bool
min_wakeup_ticks(const struct list_elem *a, const struct list_elem *b, void *aux UNUSED)
{
...
void updatePriorities(int64_t ticks, int64_t freq, struct thread *t)
{
    ...
    { /// to be updated each second
        int loadAvgFrac = multiplyTwoFP(makeFP_thenDivide(59, 60), loadAvg.loadAvg);
        int s = list_size(&ready_list);
        if (t != idle_thread)
            s += 1;
        int readyThreadfrac = makeFP_thenDivide(s, 60);
        ...
        for (struct list_elem *iter = list_begin(&all_list); iter != list_end(&all_list); iter = list_next(iter))
        {
            ...
            {
                loadAvgFrac = divideTwoFP(makeFirstFP_thenMultiply(2, loadAvg.loadAvg),
